...
@pytest.mark.asyncio
async def test_discord_free_response_channel_auto_threads_top_level_posts(adapter, monkeypatch):
    """Free-response channels auto-thread top-level posts by default.

    Mention-free channels are often used as "main" intake lanes: every new
    top-level post should immediately get an isolated thread, while quote
    replies and channels listed in DISCORD_NO_THREAD_CHANNELS remain inline.
    """
    monkeypatch.setenv("DISCORD_REQUIRE_MENTION", "true")
